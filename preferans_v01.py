...
def choose_bot_bid(
    ...
    if defend_previous_bid:
        ...
        for item in evaluations:
            ...
            # Умный риск.
            #
            # Очень слабые заявки не разрешаем,
            # даже если confidence оказался высоким.
            # ------------------------------------------------
            ...
            # ------------------------------------------------
            # Смотрим, насколько среднее количество
            # взяток отстаёт от уровня контракта.
            #
            # Небольшой недобор допускает риск:
            #
            # 7♥ при среднем 6.04
            # недобор = 0.96
            #
            # Это уже пограничный, но разумный риск.
            # ------------------------------------------------
            ...
            # confidence >= 25
            # вероятность >= 20%
            # средний недобор не более 1 взятки.
            # ------------------------------------------------

            if (
                confidence >= 25
                and probability >= 0.20
                and average_deficit <= 1.0
            ):
            ...
            # ------------------------------------------------
            # Более осторожный риск.
            #
            # Если confidence ниже, требуем вероятность
            # не менее 30%.
            # ------------------------------------------------
...
def bot_make_play():
    ...
    if suit == "Мизер":
        ...
        if player == declarer:
            ...
            # Любой защитник против Мизера
            # играет именно против разыгрывающего.
            #
            # Главная цель — заставить декларанта
            # взять взятку.
            goal_state = "MIZER_DEFENSE"
    ...
    def evaluate_card(card):
        ...
        if goal_state == "NEED_TRICKS":
            ...
            if wins_now:
                ...
                # ------------------------------------------------
                # ПЛАН ВЗЯТОК ДЛЯ ДЕКЛАРАНТА
                #
                # Здесь ИИ должен смотреть не только на
                # текущую взятку, но и на то, что останется
                # после неё.
                #
                # Например:
                #
                ...
                # осталось 6 карт
                #
                # После текущей взятки останется:
                #
                # нужно ещё 3
                # карт ещё 5
                #
                # Поэтому текущую взятку нельзя оценивать
                # отдельно от оставшейся руки.
                # ------------------------------------------------
                ...
                if player == declarer:
                    ...
                    # ------------------------------------------------
                    # future — уже существующая оценка будущей руки.
                    #
                    # Она учитывает оставшиеся сильные карты,
                    # козыри и короткие масти.
                    #
                    # Здесь используем её именно как запас
                    # возможностей для дальнейшего добора.
                    # ------------------------------------------------

                    future_after = 0

                    for future_card in hand:

                        if future_card == card:
                            continue

                        future_after += future_value(
                            future_card
                        )

                    # ------------------------------------------------
                    # Чем меньше запас будущих возможностей
                    # относительно оставшихся обязательных
                    # взяток, тем опаснее текущая ситуация.
                    # ------------------------------------------------
                    ...
                    if (
                        ...
                        if required_ratio >= 0.80:

                            score += 65

                        elif required_ratio >= 0.66:

                            score += 45

                        elif required_ratio >= 0.50:

                            score += 25

                    # ------------------------------------------------
                    # Если после текущей взятки ИИ должен брать
                    # почти все оставшиеся карты, сохранение
                    # будущих возможностей становится особенно
                    # важным.
                    #
                    # future_after — не количество взяток,
                    # поэтому сравниваем его с запасом
                    # качественно, а не как точное число.
                    # ------------------------------------------------
        ...
        # ----------------------------------------------------
        # Защита против Мизера
        # ----------------------------------------------------

        elif goal_state == "MIZER_DEFENSE":

            current_tricks = tricks_won[player]

            # ------------------------------------------------
            # Главная цель защиты против Мизера:
            #
            # 1. Лучше всего — взятка у Мизерщика.
            #
            # 2. Если взятку получает другой защитник —
            #    это допустимо.
            #
            # 3. Если взятку получает сам ИИ —
            #    это плохо.
            # ------------------------------------------------
            ...
            if simulated_winner == declarer:
                ...
                # ------------------------------------------------
                # Мизерщик уже является текущим победителем
                # среди сыгранных карт.
                # ------------------------------------------------

                score += 160

                # Первая взятка Мизерщика особенно ценна.
                if tricks_won[declarer] == 0:

                    score += 45

                # Слабой картой добиться этого предпочтительнее.
                score += (
                    35
                    - current_strength * 3
                )
                ...
                # ------------------------------------------------
                # Взятка пока у другого защитника.
                #
                # Это лучше собственной взятки,
                # но хуже, чем отдать её Мизерщику.
                # ------------------------------------------------
            ...
            elif simulated_winner == player:

                # ------------------------------------------------
                # Сам ИИ забирает взятку.
                #
                # Для защиты Мизера это наиболее нежелательный
                # вариант: теперь именно он будет вести дальше.
                # ------------------------------------------------
            ...
            # ------------------------------------------------
            # Если в текущей взятке уже есть карта Мизерщика,
            # пытаемся оценить, насколько вероятно, что после
            # нашего хода именно он сможет получить взятку.
            #
            # Реальные карты соперников НЕ смотрим.
            # ------------------------------------------------
            ...
            if trick_cards:
                ...
                for trick_player, trick_card in trick_cards:

                    if trick_player == declarer:

                        declarer_card = trick_card

                        break

                # ------------------------------------------------
                # Если карта Мизерщика уже лежит во взятке,
                # проверяем неизвестные карты той же масти.
                # ------------------------------------------------

                if declarer_card is not None:
                    ...
                    for _, played_card in trick_cards:
                    ...
                    for unknown_card in unknown_cards:
                        ...
                        if (
                            ...
                            if (
                                ...
                                stronger_than_declarer += 1

                    # ------------------------------------------------
                    # Чем меньше неизвестных старших карт осталось
                    # в масти Мизерщика, тем выше шанс его взятки.
                    # ------------------------------------------------

                    if same_suit_unknown > 0:

                        mizer_danger = (
                            1
                            - (
                                stronger_than_declarer
                                / same_suit_unknown
                            )
                        )
                    ...
                    # ------------------------------------------------
                    # Если Мизерщик уже лежит под атакой
                    # и старших неизвестных карт мало,
                    # сильно повышаем привлекательность такого
                    # варианта.
                    # ------------------------------------------------
                    ...
                    if mizer_danger >= 0.75:

                        score += 30

                    elif mizer_danger >= 0.50:

                        score += 15

                    elif mizer_danger < 0.25:
            ...
            # Первый ход.
            #
            # Здесь Мизерщик ещё не сыграл карту.
            # Поэтому сохраняем осторожный человеческий выбор:
            # не выбрасывать сильную карту и предпочитать
            # короткие масти.
            # ------------------------------------------------
            ...
            if not trick_cards:
                ...
                # ------------------------------------------------
                # Средние карты интереснее крайних.
                #
                # Слишком слабая карта часто просто отдаёт
                # взятку другому защитнику.
                # Слишком сильная карта может взять её самому.
                # ------------------------------------------------
            ...
            # ------------------------------------------------
            # Если ИИ уже получил взятку,
            # теперь особенно важно избавиться от следующей
            # в пользу Мизерщика.
            # ------------------------------------------------
        ...
        if not trick_cards:
            ...
            if goal_state == "NEED_TRICKS":
                ...
                # Защита против Мизера.
                #
                # На первом выходе ещё невозможно определить
                # победителя текущей взятки, поэтому ищем
                # масть и карту, которые разумно отдавать
                # в сторону разыгрывающего.
                #
                # Слишком сильную карту отдавать не хочется:
                # если Мизер не сможет взять, мы сами можем
                # получить взятку.
                # ------------------------------------------------
                ...
                # Короткая масть полезна:
                # после её окончания можно будет
                # избавляться от опасных карт.
                if suit_count == 1:
                ...
                # ------------------------------------------------
                # Но против Мизера особенно опасны
                # слишком слабые выходы в масть,
                # где у разыгрывающего могут быть
                # карты чуть старше.
                #
                # Поэтому небольшой бонус оставляем
                # картам средней силы.
                # ------------------------------------------------
                ...
                # Очень сильные карты нежелательны
                # для первого выхода.
                if current_strength >= 7:
